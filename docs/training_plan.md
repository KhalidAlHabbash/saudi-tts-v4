...
`configs/train.yaml` specifies the local profile. The RunPod profile in `configs/train_runpod.yaml` continues the full update-100 state on one Secure RTX PRO 4500 Blackwell using BF16, batch 2, accumulation 4, <=8-second clips, activation checkpointing, ordinary PyTorch attention/AdamW, four workers, pinned memory, and TF32. The scheduler ceiling remains 120,000 updates; stage stops are 13,122, 26,144, 52,188, 104,276, and 120,000. Validation runs every 250 updates, `model_last` around every 500, and immutable checkpoints every 5,000 plus stage boundaries.
